...
This section will describe the external output of your webpage. For full credit, your solution should adhere to *both* the visual references (provided throughout spec) and text specifications.  We have provided [screenshots](https://courses.cs.washington.edu/courses/cse154/21sp/homework/hw4/screenshots/) and a [video](https://courses.cs.washington.edu/courses/cse154/21sp/homework/hw4/hw4.mp4) with example of the functionality of a fully implemented Yipper site. Note that, based on your OS and browser, the page may render slightly differently than the provided screenshots/video. If you think the specification is missing a requirement, make sure to review both text-based and visual guides and also confirm that the requirement isn't covered implicitly by another requirement. If you are unsure, post to Ed with clarifying questions.
...
#### Search Bar Behavior
When typing into the `#search-term` bar, the `#search-btn` should only be enabled when the trimmed (of whitespace) value in the search bar is not empty. If it is empty, the `#search-btn` should be disabled. For example, if the characters typed into the `#search-term` were `         ` (a bunch of whitespace, which, when trimmed down is an empty string), the `#search-btn` should remain disabled. Otherwise, it should be enabled.
  * *Hint*: the `input` event will be helpful in accomplishing this behavior.
...
* Similar to your client-side JS, decompose your Node.js/Express API by writing smaller, more generic functions that complete one task rather than a few larger "do-everything" functions. You are expected to define original helper functions for your web service (provided function(s) are not considered original).
...
* In your Node.js file, your file header comment should additionally provide a descriptive summary of the web service and description of all endpoints (endpoint format, parameters, description of endpoint, response content type, and possible errors). This file comment is more descriptive than other JS file comments since you are not providing an APIDOC.
